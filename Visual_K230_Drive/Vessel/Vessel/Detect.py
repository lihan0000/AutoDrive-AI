...
def arr_Handle(arr, C_X, C_Y):
    sliced_arr = arr[:, :2]
    C_arr = np.array([C_X, C_Y])
    L_list = []
    R_list = []
    L_arr = np.array([0, 0])
    R_arr = np.array([0, 0])
    for i in range(sliced_arr.shape[0]):
        if sliced_arr[i][0] < C_arr[0]:
            L_list.append(sliced_arr[i])
        if sliced_arr[i][0] > C_arr[0]:
            R_list.append(sliced_arr[i])
    if not L_list or not R_list:
        ...
        for i in range(len(L_list)):
            if L_arr[1] > L_list[i][1]:
                L_arr = L_list[i]
            else:
                if L_arr[0] < L_list[i][0]:
                    L_arr = L_list[i]
        for i in range(len(R_list)):
            if R_arr[1] > R_list[i][1]:
                R_arr = R_list[i]
            else:
                if R_arr[0] > R_list[i][0]:
                    R_arr = R_list[i]
        return L_arr, R_arr
...
if __name__ == "__main__":
    ...
    kmodel_path = "/sdcard/app/tests/kmodel/test_Vessel.kmodel"
    labels = ["Person"]
    ...
    try:
        while True:
            ...
            if len(res) != 0:
                res = np.array(res)
                if res.shape[1] > 1:
                    if arr_Handle(res, 320, 320):
                        arr1, arr2 = arr_Handle(res, 320, 320)
                        if ((arr1[1] + arr2[1])/2) < 320:
                            arrA = 320 - ((arr1[1] + arr2[1]) / 2)
                            print(0)
                            print(arrA)
                        else:
                            arrA = ((arr1[1] + arr2[1]) / 2) - 320
                            print(1)
                            print(arrA)
                else:
                    print("FF")
